...
class AppMenu extends React.Component {
  ...
  getSubMenuOrItem = item => {    
    if (item.children && item.children.some(child => child.name)) {
      ...
      return <Menu.Item key={item.path}><Button className="nav-item" href={'#' + item.path}><MaterialIcon icon="arrow_right" style={{color: '#00C853'}} /><span>{item.menuName || item.name}</span></Button></Menu.Item>;
    }
  };
